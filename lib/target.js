...
function external_build(steps, callback) {
	...
	if (this.isDirty) {
		function next(i, items) {
			if (self.steps.length > i) {
				...
				self.isDirty = false;
				self.output = items;
				self.external_build(self.options.after_build, callback);
			}
		}

		self.external_build(self.options.before_build, function (err) {
			if (err) return callback(err);
			next(0, self.items);
		});
	} else {
...
Target.prototype.external_build = external_build;
