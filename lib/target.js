...
export class Target extends EventEmitter {
  ...
  externalBuild (steps, callback) {
    ...
    let cmds = modelResolve(step, this.options)
    ...
    let next = (i) => {
      if (cmds.length > i) {
        var cwd = this.options.rootdir
        var env = assign({}, process.env, modelResolve(this.options, this.options), this.options[process.platform])
        var cmd = cmds[i]
