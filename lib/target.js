...
export class Target extends EventEmitter {
  ...
  externalBuild (steps, callback) {
    ...
    let cmds = modelResolve(steps, this.options)
    if (!Array.isArray(cmds)) {
      cmds = [cmds]
    }
    let next = (i) => {
      if (cmds.length > i) {
        const cwd = this.options.rootdir
        const env = assign({}, process.env, modelResolve(this.options, this.options), this.options[process.platform])
        const cmd = cmds[i]
        const args = {
          cmd,
          cwd,
