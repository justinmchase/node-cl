...
function buildLib(target, options, callback) {
	...
	function next() {
		if (sources.length) {
			...
			var cl_opts = {
				noLink: true,
				outfile: path.join(options.outdir, options.configuration, options.arch, 'objects', path.basename(source, path.extname(source)) + '.obj'),
				sources: [source],
				//arch: options.arch
			};
			...
			var lib_opts = {
				outfile: path.join(options.outdir, options.configuration, options.arch, target.target + '.lib'),
				sources: objects,
			};
	...
	// build all sources with clang-cl /c into .obj's
	// build all obj's llvm-lib into .lib
}
...
function buildExe(target, options, callback) {
	...
	var cl_opts = {
		outfile: path.join(options.outdir, options.configuration, options.arch, target.target + '.exe'),
	...
	if (target.dependencies && target.dependencies.length) {
		for (var i = 0, n = target.dependencies.length; i < n; i++) {
			...
			cl_opts.sources.push(path.join(options.outdir, options.configuration, options.arch, depname + '.lib'));
		}
	}
